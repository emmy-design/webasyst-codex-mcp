...
async function findWebasystRoot(startCwd = process.cwd()) {
	if (process.env.WEBASYST_ROOT) {
		const rootFromEnv = path.resolve(process.env.WEBASYST_ROOT);
		const indexPath = path.join(rootFromEnv, 'index.php');
		const systemPath = path.join(rootFromEnv, 'wa-system');
		if (await fileExists(indexPath) && await fileExists(systemPath)) {
			return rootFromEnv;
		}
		throw new Error(`WEBASYST_ROOT задан, но не похож на корень Webasyst: ${rootFromEnv}`);
	}
...
// ========= Codex-first UI context tools =========
function sanitizeFileName(name) {
	return String(name || '').replace(/[^a-zA-Z0-9_-]/g, '').toLowerCase();
}

async function resolveFromRoot(inputPath) {
	const rootPath = await findWebasystRoot();
	const resolvedPath = path.isAbsolute(inputPath) ? inputPath : path.join(rootPath, inputPath);
	return { rootPath, resolvedPath };
}

async function readTextIfExists(filePath, maxChars = 12000) {
	await fs.access(filePath);
	const content = await fs.readFile(filePath, 'utf-8');
	return content.length > maxChars ? `${content.slice(0, maxChars)}\n\n... truncated ...` : content;
}

function getCodexUIComponentTemplate(componentType, componentName, localizationFunction = '_w') {
	const safeName = sanitizeFileName(componentName) || 'component';
	const templates = {
		settings_page: `<div class="article">
    <div class="article-body">
        <h1>{_w('Settings')}</h1>
        <p class="hint">{_w('Configure feature behavior using Webasyst UI 2.0 controls.')}</p>

        <div class="fields">
            <div class="fields-group">
                <div class="field">
                    <div class="name for-input">{_w('Name')}</div>
                    <div class="value">
                        <input class="long bold" type="text" name="settings[name]" value="">
                        <p class="hint">{_w('Shown in the backend interface.')}</p>
                    </div>
                </div>
                <div class="field">
                    <div class="name for-checkbox">{_w('Enabled')}</div>
                    <div class="value">
                        <label>
                            <span class="wa-checkbox">
                                <input type="checkbox" name="settings[enabled]" value="1" checked>
                ...
                <div class="field">
                    <div class="value submit">
                        <button class="button blue" type="submit">{_w('Save')}</button>
                        <button class="button light-gray" type="button">{_w('Cancel')}</button>
                    </div>
                </div>
            </div>
        </div>
...
`,
    ...
    <div class="fields-group">
        <div class="field">
            <div class="name for-input">{_w('Title')}</div>
            <div class="value">
                <input class="long bold" type="text" name="${safeName}[title]" value="">
            </div>
        </div>
        <div class="field vertical">
            <div class="name for-input">{_w('Description')}</div>
            <div class="value">
                <textarea class="wide" name="${safeName}[description]"></textarea>
            </div>
        </div>
        <div class="field">
            <div class="value submit">
...
`,
		table: `<div class="table-scrollable-x">
    <table class="zebra single-lined">
        <thead>
            <tr>
                <th class="min-width"></th>
                <th><span class="nowrap">{_w('Name')} <i class="fas fa-caret-down"></i></span></th>
                <th>{_w('Status')}</th>
                <th class="align-right">{_w('Actions')}</th>
            </tr>
        </thead>
        <tbody>
            {foreach $items as $item}
                <tr>
                    <td class="min-width"><input type="checkbox" value="{$item.id|escape}"></td>
                    <td>{$item.name|escape}</td>
                    <td><span class="badge green">{_w('Active')}</span></td>
                    <td class="align-right">
                        <button class="button small light-gray" type="button"><i class="fas fa-edit"></i></button>
                    </td>
                </tr>
            {/foreach}
        </tbody>
...
`,
		dialog: `<div class="dialog" id="${safeName}-dialog">
    <div class="dialog-background"></div>
    <div class="dialog-body">
        <a href="#" class="dialog-close js-close-dialog"><i class="fas fa-times"></i></a>
        <header class="dialog-header"><h1>{_w('Edit')}</h1></header>
        <div class="dialog-content">
            <div class="fields vertical">
                <div class="field">
                    <div class="name for-input">{_w('Name')}</div>
                    <div class="value"><input class="long bold" type="text" name="name"></div>
                </div>
            </div>
        </div>
        <footer class="dialog-footer">
            <button class="button blue js-submit-dialog" type="button">{_w('Save')}</button>
            <button class="button light-gray js-close-dialog" type="button">{_w('Cancel')}</button>
        </footer>
    </div>
</div>
<script>
    (function($) {
        $("#${safeName}-dialog").waDialog();
    })(jQuery);
</script>
`,
		tabs: `<ul class="tabs bordered-bottom">
    <li class="selected"><a href="#/${safeName}/">{_w('Overview')}</a></li>
    <li><a href="#/${safeName}/settings/">{_w('Settings')}</a></li>
    <li><a href="#/${safeName}/history/">{_w('History')} <span class="count">0</span></a></li>
</ul>
`,
		empty_state: `<div class="box align-center">
    <div>
        <i class="fas fa-inbox text-light-gray" style="font-size: 3rem;"></i>
        <h2>{_w('Nothing here yet')}</h2>
        <p class="hint">{_w('Create the first item to start working.')}</p>
        <button class="button blue" type="button"><i class="fas fa-plus"></i> {_w('Create')}</button>
    </div>
</div>
`
	};
	...
	if (!templates[componentType]) {
		throw new Error(`Неизвестный тип UI компонента: ${componentType}`);
	}

	return templates[componentType].replaceAll('{_w(', `{${localizationFunction}(`);
}

async function listWebasystUIComponentsTool({ query = '' } = {}) {
	const rootPath = await findWebasystRoot();
	const componentsPath = path.join(rootPath, 'wa-apps', 'ui', 'templates', 'actions', 'component');
	const jsPath = path.join(rootPath, 'wa-apps', 'ui', 'js');
	const components = (await fs.readdir(componentsPath))
		.filter((file) => file.endsWith('.html'))
		.map((file) => file.replace(/\.html$/, ''))
		.filter((name) => !query || name.includes(query));
	const jsHelpers = (await fs.readdir(jsPath)).filter((file) => file.endsWith('.js'));

	return { content: [{ type: 'text', text: [
		'Webasyst UI 2.0 источники:',
		`- CSS: ${path.join(rootPath, 'wa-content', 'css', 'wa', 'wa-2.0.css')}`,
		`- Компоненты: ${componentsPath}`,
		`- JS helpers: ${jsPath}`,
		'',
		...
		'',
		'JS helpers:',
		jsHelpers.map((name) => `- ${name}`).join('\n')
	].join('\n') }] };
}

async function getWebasystUIComponentTool({ component, max_chars = 12000 }) {
	const safeComponent = sanitizeFileName(component);
	if (!safeComponent) {
		throw new Error('Имя компонента пустое или некорректное');
	}

	const rootPath = await findWebasystRoot();
	const componentPath = path.join(rootPath, 'wa-apps', 'ui', 'templates', 'actions', 'component', `${safeComponent}.html`);
	const content = await readTextIfExists(componentPath, max_chars);
	return { content: [{ type: 'text', text: `Источник компонента ${safeComponent}:\n${componentPath}\n\n${content}` }] };
}

async function getWebasystUIContextTool({ components = ['button', 'fields', 'table', 'dialog', 'dropdown', 'tabs', 'sidebar'], include_examples = true } = {}) {
	const rootPath = await findWebasystRoot();
	const componentDir = path.join(rootPath, 'wa-apps', 'ui', 'templates', 'actions', 'component');
	const cssPath = path.join(rootPath, 'wa-content', 'css', 'wa', 'wa-2.0.css');
	const jsPath = path.join(rootPath, 'wa-apps', 'ui', 'js');
	const componentNames = components.map((name) => sanitizeFileName(name)).filter(Boolean);
	const examples = [];

	if (include_examples) {
		for (const name of componentNames) {
			const filePath = path.join(componentDir, `${name}.html`);
			try {
				examples.push(`--- ${name}.html ---\n${await readTextIfExists(filePath, 2500)}`);
			} catch {
				examples.push(`--- ${name}.html ---\nКомпонент не найден: ${filePath}`);
			}
		}
	}

	const text = [
		'WEBASYST UI 2.0 CONTRACT FOR ALL APP/PLUGIN UI WORK',
		'',
		'Перед созданием или редактированием backend-экранов приложений и плагинов:',
		'1. Используй реальные паттерны из wa-apps/ui/templates/actions/component/*.html.',
		'2. Используй классы из wa-content/css/wa/wa-2.0.css: article, article-body, flexbox, wrap-mobile, content, box, blank, fields, fields-group, field, name, value, button, tabs, table-scrollable-x, zebra, single-lined, dropdown, dialog, badge, hint.',
		'3. Для форм используй .fields > .fields-group > .field > .name/.value; для кнопок .button blue/light-gray/red/outlined; для таблиц .table-scrollable-x + table.zebra; для вкладок ul.tabs; для диалогов .dialog + $.waDialog().',
		'4. Для интерактива используй wa-content/js/jquery-wa/wa.js и helpers из wa-apps/ui/js: dialog.js, dropdown.js, tooltip.js, toggle.js, tabs via wa.js.',
		'5. Не вводи Bootstrap/Tailwind/произвольные design-system классы. Кастомный CSS допускается только для уникальной компоновки, сначала ищи готовый класс в wa-2.0.css.',
		'6. Тексты в PHP локализуй _w() для приложений и _wp() для плагинов; в Smarty используй {_w(...)} / {_wp(...)} по контексту; в JS используй $_() если локализация уже проброшена.',
		'',
		`Root: ${rootPath}`,
		`CSS: ${cssPath}`,
		`Компоненты: ${componentDir}`,
		`JS helpers: ${jsPath}`,
		'',
		'Минимальные сниппеты:',
		'<button class="button blue" type="button"><i class="fas fa-plus"></i> {_w(\'Create\')}</button>',
		'<div class="fields"><div class="fields-group"><div class="field"><div class="name for-input">{_w(\'Name\')}</div><div class="value"><input class="long bold" type="text"></div></div></div></div>',
		'<div class="table-scrollable-x"><table class="zebra single-lined">...</table></div>',
		'<ul class="tabs bordered-bottom"><li class="selected"><a href="#">...</a></li></ul>',
		'',
		include_examples ? examples.join('\n\n') : ''
	].join('\n');

	return { content: [{ type: 'text', text }] };
}

async function createWebasystUIComponentTool({ target_path, component_type, component_name, localization = 'app' }) {
	const { resolvedPath } = await resolveFromRoot(target_path);
	const componentsPath = path.join(resolvedPath, 'templates', 'components');
	const safeName = sanitizeFileName(component_name);
	if (!safeName) {
		throw new Error('Имя компонента пустое или некорректное');
	}

	await ensureDir(componentsPath);
	const targetFile = path.join(componentsPath, `${safeName}.html`);
	const template = getCodexUIComponentTemplate(component_type, safeName, localization === 'plugin' ? '_wp' : '_w');
	await fs.writeFile(targetFile, template);
	return { content: [{ type: 'text', text: `UI 2.0 компонент создан: ${targetFile}\nТип: ${component_type}\n\nПроверь его через audit_webasyst_ui_surface и при необходимости сверяйся с get_webasyst_ui_component.` }] };
}

async function auditWebasystUISurfaceTool({ file_path }) {
	const { rootPath, resolvedPath } = await resolveFromRoot(file_path);
	const content = await readTextIfExists(resolvedPath, 200000);
	const isFullSurface = /<html\b|<head\b|<body\b|\{\$wa->header\(\)\}/i.test(content);
	const hasFormControls = /<form\b|<textarea\b|<select\b|<input\b(?![^>]*type=["']?checkbox)(?![^>]*type=["']?radio)/i.test(content);
	const checks = [
		{
			name: 'Webasyst layout/classes',
			ok: /\b(article|content|box|flexbox|fields|button|tabs|dialog|dropdown|zebra|table-scrollable-x)\b/.test(content),
			hint: 'Добавь стандартные классы Webasyst UI 2.0 вместо кастомной сетки.'
		},
		{
			name: 'wa-2.0.css / $wa->css()',
			ok: !isFullSurface || /wa-2\.0\.css|\{\$wa->css\(\)\}/.test(content),
			hint: 'Backend layout должен загружать {$wa->css()} или wa-content/css/wa/wa-2.0.css.'
		},
		{
			name: 'wa.js / UI JS helpers',
			ok: !isFullSurface || /jquery-wa\/wa\.js|waDialog|waDropdown|waTooltip|waTabs/.test(content),
			hint: 'Для диалогов, dropdown, tooltip и tabs используй Webasyst helpers, не нативные alert/confirm.'
		},
		{
			name: 'No Bootstrap/Tailwind classes',
			ok: !/\b(btn-primary|btn-secondary|row|col-md-|container-fluid|text-center|bg-|rounded-lg|shadow-lg|p-\d|m-\d|grid-cols-)\b/.test(content),
			hint: 'Замени Bootstrap/Tailwind-like классы на wa-2.0.css.'
		},
		{
			name: 'No native alert/confirm/prompt',
			ok: !/\b(alert|confirm|prompt)\s*\(/.test(content),
			hint: 'Используй $.waDialog.alert() / $.waDialog.confirm() или полноценный .dialog.'
		},
		{
			name: 'Forms use .fields',
			ok: !hasFormControls || /\bfields\b/.test(content),
			hint: 'Формы должны использовать .fields, .field, .name, .value.'
		},
		{
			name: 'Tables are scroll-safe',
			ok: !/<table\b/i.test(content) || /table-scrollable-x|class="[^"]*\bzebra\b/.test(content),
			hint: 'Таблицы оборачивай в .table-scrollable-x и используй table.zebra/single-lined.'
		}
	];

	const failed = checks.filter((check) => !check.ok);
	const report = checks.map((check) => `${check.ok ? 'OK' : 'FIX'} ${check.name}${check.ok ? '' : ` - ${check.hint}`}`).join('\n');
	return { content: [{ type: 'text', text: [
		`UI audit: ${resolvedPath}`,
		`Root: ${rootPath}`,
		'',
		report,
		'',
		failed.length ? `Нужно исправить пунктов: ${failed.length}` : 'Экран выглядит совместимым с Webasyst UI 2.0 по базовым проверкам.'
	].join('\n') }] };
}
...
class WebasystMCPServer {
	constructor() {
		this.server = new Server(
			{ name: 'webasyst-codex-mcp', version: '2.3.0' },
	...
	setupHandlers() {
		// Codex may probe resources/list and prompts/list even for tool-only MCP servers.
		// Return empty lists so the handshake stays quiet and deterministic.
		...
		this.server.setRequestHandler(ListToolsRequestSchema, async () => ({
			tools: [
				...
				{ name: 'get_webasyst_ui_context', description: 'Codex-first обязательный контекст Webasyst UI 2.0 для создания и редактирования backend-экранов: правила, пути, классы wa-2.0.css и выдержки из wa-apps/ui', inputSchema: { type: 'object', properties: { components: { type: 'array', items: { type: 'string' }, description: 'Компоненты из wa-apps/ui/templates/actions/component без .html, например button, fields, table, dialog' }, include_examples: { type: 'boolean', default: true, description: 'Добавить короткие выдержки из реальных component/*.html' } } } },
				{ name: 'list_webasyst_ui_components', description: 'Показать доступные компоненты wa-apps/ui и JS-хелперы Webasyst UI 2.0; используй перед выбором UI-паттерна', inputSchema: { type: 'object', properties: { query: { type: 'string', description: 'Фильтр по имени компонента' } } } },
				{ name: 'get_webasyst_ui_component', description: 'Получить исходник конкретного компонента из wa-apps/ui/templates/actions/component/*.html для точного повторения паттерна', inputSchema: { type: 'object', properties: { component: { type: 'string', description: 'Имя компонента без .html' }, max_chars: { type: 'number', default: 12000, description: 'Максимум символов исходника' } }, required: ['component'] } },
				{ name: 'create_webasyst_ui_component', description: 'Создать Codex-friendly шаблон UI 2.0 компонента в target_path/templates/components с классами из wa-2.0.css', inputSchema: { type: 'object', properties: { target_path: { type: 'string', description: 'Путь к приложению/плагину или относительный путь от корня Webasyst' }, component_type: { type: 'string', enum: ['settings_page', 'form', 'table', 'dialog', 'tabs', 'empty_state'], description: 'Тип компонента' }, component_name: { type: 'string', description: 'Имя файла компонента без .html' }, localization: { type: 'string', enum: ['app', 'plugin'], default: 'app', description: 'Какую Smarty-функцию локализации использовать: app -> _w(), plugin -> _wp()' } }, required: ['target_path', 'component_type', 'component_name'] } },
				{ name: 'audit_webasyst_ui_surface', description: 'Проверить HTML/Smarty backend-шаблон на базовое соответствие Webasyst UI 2.0 после правок', inputSchema: { type: 'object', properties: { file_path: { type: 'string', description: 'Путь к HTML/Smarty шаблону, абсолютный или относительный от корня Webasyst' } }, required: ['file_path'] } },
		...
		this.server.setRequestHandler(CallToolRequestSchema, async (request) => {
			...
			try {
				switch (name) {
					...
					case 'get_webasyst_ui_context': return await getWebasystUIContextTool(args);
					case 'list_webasyst_ui_components': return await listWebasystUIComponentsTool(args);
					case 'get_webasyst_ui_component': return await getWebasystUIComponentTool(args);
					case 'create_webasyst_ui_component': return await createWebasystUIComponentTool(args);
					case 'audit_webasyst_ui_surface': return await auditWebasystUISurfaceTool(args);
	...
	async run() {
		...
		console.error('Webasyst Codex MCP Server v2.3.0 started');
	}
}
